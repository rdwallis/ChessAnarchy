...
public class MoveTree {
    ...
    private boolean calculateIsCheck(final boolean swapSides) {
        final Map<Integer, Integer> castleKingPositions;
        if (swapSides) {
            if (pgn.equals("O-O")) {
                castleKingPositions = new HashMap<Integer, Integer>();
                castleKingPositions.put(4, move.getStartRank());
                castleKingPositions.put(5, move.getStartRank());
            } else if (pgn.equals("O-O-O")) {
                castleKingPositions = new HashMap<Integer, Integer>();
                castleKingPositions.put(4, move.getStartRank());
                castleKingPositions.put(3, move.getStartRank());
            } else {
                castleKingPositions = null;
            }
        } else {
            castleKingPositions = null;
        }
        for (int file = 0; file < 8; file++) {
            for (int rank = 0; rank < 8; rank++) {
                if (isCastleKingPosition(file, rank, castleKingPositions) || (isKing(file, rank) && (swapSides ^ isMyPiece(file, rank)))) {
    ...
    private boolean isCastleKingPosition(final int file, final int rank, final Map<Integer, Integer> castleKingPositions) {
        return (castleKingPositions != null && castleKingPositions.containsKey(file) && castleKingPositions.get(file) == rank);
    }
