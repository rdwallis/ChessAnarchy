...
/**
 * Generator to enable/disable {@link UserAgentAsserter}. This generator exists
 * because we can't deferred-bind via configuration property.
 */
public class UserAgentAsserterGenerator extends Generator {
    ...
    private static final String USER_AGENT_ASSERTER_DISABLED = UserAgentAsserterDisabled.class.getCanonicalName();
    ...
    public String generate(final TreeLogger logger, final GeneratorContext context, final String typeName) throws UnableToCompleteException {
        try {
            final ConfigurationProperty property = context.getPropertyOracle().getConfigurationProperty(PROPERTY_USER_AGENT_RUNTIME_WARNING);
